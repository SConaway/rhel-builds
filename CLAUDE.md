...
- `tag-on-merge.yml` needs a `RELEASE_TAG_PAT` repo secret (Settings → Secrets and variables → Actions): a personal access token (classic or fine-grained, `contents: write` on this repo) used to push the release tag. **This is required, not optional** — tags pushed with the default `GITHUB_TOKEN` are attributed to `github-actions[bot]` and GitHub's loop-prevention rule silently blocks them from triggering other workflows' `on.push.tags`, so the per-package release workflow never fires. A PAT belonging to a real user avoids this.
